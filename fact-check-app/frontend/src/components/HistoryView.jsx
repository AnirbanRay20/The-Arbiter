...
export default function HistoryView({ onSelect, onGoToDashboard }) {
  ...
  const [sortBy, setSortBy]     = useState('date'); // 'date' | 'accuracy' | 'risk'
  ...
  const shareItem = (e, id) => {
    ...
    if (item) navigator.clipboard.writeText(item.q);
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      style={{ width: '100%', maxWidth: 900, margin: '0 auto', padding: '2.5rem 1.5rem' }}
    >
      {/* ── Header ── */}
      ...
      {search && history.length > 0 && (
        <div style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontFamily: 'IBM Plex Mono', fontSize: 10, color: '#bac9cc' }}>
            ...
            <span style={{ color: '#00E5FF' }}>{search}</span>"
          </span>
          <button onClick={() => setSearch('')} style={{
      ...
      {history.length === 0 ? (
        <div style={{
          ...
          justifyContent: 'center', minHeight: '50vh', textAlign: 'center', gap: '1.25rem',
        }}>
        ...
        <div style={{
          display: 'flex', flexDirection: 'column', alignItems: 'center',
          justifyContent: 'center', minHeight: '35vh', textAlign: 'center', gap: '1rem',
        }}>
...
function HistoryItem({ item, index, searchQuery, onSelect, onDelete, onShare, onStar }) {
  ...
  // Highlight search match in query text
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
        ...
        {/* Star */}
        <button onClick={(e) => onStar(e, item.id)} title={item.starred ? 'Unstar' : 'Star'} style={{
        ...
        {/* Share + Delete on hover */}
        <div style={{ display: 'flex', gap: 5, opacity: hovered ? 1 : 0, transition: 'opacity 0.2s' }}>
          <button onClick={(e) => onShare(e, item.id)} title="Copy query" style={{
          ...
          <button onClick={(e) => onDelete(e, item.id)} title="Delete" style={{
            width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center',
            backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid transparent',
            borderRadius: 6, cursor: 'pointer', color: '#bac9cc', transition: 'all 0.15s',
          }}
